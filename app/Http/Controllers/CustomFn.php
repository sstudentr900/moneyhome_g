...
class CustomFn
{
  ...
  static function imgAdd($fileValue, $customName, $fileSrc=null)
  {
    ...
    // $fileSrc = $fileSrc?$fileSrc:config('app.imgName');
    $fileSrc = $fileSrc?$fileSrc:env('MIX_IMGPREFIX','img');
    $path = base_path().'/public/'.$fileSrc.'/'.$imgName; //圖片路徑
    // dd($path);
    // $path = base_path().$fileSrc.$imgName; //圖片路徑
  ...
  //西元年轉民國年
  //https://vocus.cc/article/6194774efd89780001c97898
  static function toChinaDate($date){
    // $date = "1990-04-19";
    $day = (new DateTime($date))->modify("-1911 year")->format('Y-m-d');
    //$day = "0079-04-19"
    $day = ltrim($day, 0);  //去掉左邊的0
    //$day = "79-04-19"
    $day = str_replace("-", "/", $day);
    //$day = "79/04/19"
    return $day;
