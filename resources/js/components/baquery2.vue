<script setup>
  ...
  import badelete from './badelete2.vue'
  import baadd from '../components/baadd2.vue'
  ...
  const show = ref('baquery')
  const loadingShow = ref(false)
  const nowdata = ref({})
  const searchFn = async(now=route.params.id)=>{
    try{
      const result = await props.searchApi(now);
      if(result.status){
        // console.log(result.pageNow,props.pagename)
        nowdata.value = result.datas; 
        router.push({ name: `${props.pagename}`, params:{id:result.pageNow} })
      }else{
        console.log('找不到資料')
      }
    }catch(eror){
  ...
  const addFn = async(values,actions)=>{
    ...
    try{
      ...
      const result = await addApi(values);
...
<template>
  <div class="baquery" v-if="show=='baquery'">
    ...
    <div class="tableDiv">
      ...
      <div class="tableTitle_dif_aic">
        <div v-for="(item, index) in props.nowtable" :key="index" :class="`${item.en}Div`">
          <div v-if="item.search"><p>{{item.ch}}</p></div>
        </div>
      </div>
      <div class="tableContent_dif_aic" v-for="(item, index) in nowdata" :key="index">
        <div v-for="(item1, index1) in props.nowtable" :key="index1" :class="`${item1.en}Div`">
          <div v-if="item1.search && item1.en=='id'">
            <p>{{ item[item1.en] }}</p>
          </div>
          <div v-else-if="item1.search && item1.en=='sort'">
            <p>{{ item[item1.en] }}</p>
          </div>
          <div v-else-if="item1.search && item1.en=='title'">
            <p>{{ item[item1.en] }}</p>
          </div>
          <div v-else-if="item1.search && item1.en=='account'">
            <p>{{ item[item1.en] }}</p>
          </div>
          <div v-else-if="item1.search && item1.en=='name'">
            <p>{{ item[item1.en] }}</p>
          </div>
          <div v-else-if="item1.search && item1.en=='class'">
            <p>{{ item[item1.en] }}</p>
          </div>
          <div v-else-if="item1.search && item1.en=='updated_at'">
            <p>{{ item[item1.en] }}</p>
          </div>
          <div v-else-if="item1.search && item1.en=='releases'">
            <p class="markDiv" v-if="item[item1.en]=='y'">使用</p>
            <p class="markDiv disable" v-else>停用</p>
          </div>
          <div class='_dif' v-else-if="item1.search && item1.en=='edit'">
  ...
  <baadd v-if="show=='baadd'" :seadFn="addFn" :nowtable="nowtable" :loadingShow="loadingShow"/>
  <bakey v-model:show="passwordshow" :id="passwordid" :url="props.pagename" v-if="keymodel"/>
  <badelete v-model:show="deleteshow" :id="deleteid" :url="props.pagename" :searchFn="props.searchFn" :deletFn="props.deletFn"/>
</template>
<style scoped>
  [class^='baquery']{
    flex: 1 1;
