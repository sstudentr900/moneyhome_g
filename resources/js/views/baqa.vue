<script setup>
  ...
  import { baqa,baqaadd,baqaedit,baqaeditpost,baqasort,baqadelete } from "../composables/useApi"
  import { useRoute,useRouter } from 'vue-router'
  const route = useRoute()
  const router = useRouter()
  const pagename = ref('baqa')
  ...
  const addFn = async(values,actions)=>{
    ...
    try{
      ...
      const result = await baqaadd(values);
  ...
  const editpostFn = async(values,actions)=>{
    ...
    try{
      ...
      loadingShow.value = true
      const result = await baqaeditpost(id,values);
      loadingShow.value = false
      if(result.status){
        notify({title: result.message});
        // 成功去管員
  ...
  const editFn = async()=>{
    try{
      const id = route.params.id
      const result = await baqaedit(id);
  ...
  const deletFn = async(id,searchfn,cancelfn) => {
    try{
      loadingShow.value = true
      const result = await baqadelete(id);
      loadingShow.value = false
      if(result.status){
        notify({title: "刪除成功"});
